...
ShaderEffect{
    id: vehicleSpeedDisplay
        width: 244
        height: 447
        property real hight: -0.35
        property int speed:gaugedatamodel.gauge_VCU_VehSpd
        ...
        Behavior on hight{
          ...
          vec3 drawBox(vec2 pos,vec2 center,float len)
          {
              ...
              if(abs(pos.x-center.x)<len*2.0 && abs(pos.y-center.y)<len*1.0)
        ...
        gl_FragColor =vec4(color* (1.-d))* qt_Opacity;
        }
        "

          Image {
              id: sourceImage
              source: "images/gauge3.png"
              visible: false
          }
...
/*##^##
Designer {
    D{i:0;height:447;width:244}D{i:4}
}
##^##*/
